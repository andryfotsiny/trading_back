            ...
            for trade in open_trades:
                try:
                    ...
                    if trailing["updated"]:
                        ...
                        db.commit()
                        logger.info(f"Trailing SL mis a jour: {trade.symbol} -> {trailing['new_sl']}")
                    ...
                    if partial["should_close"] and partial["close_pct"] < 1.0:
                        ...
                        db.commit()
                        logger.info(f"Partial TP {int(partial['close_pct']*100)}%: {trade.symbol} qty={close_qty:.6f} pnl={pnl:.4f}")
